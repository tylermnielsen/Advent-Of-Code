...
int64_t scan_sequence(int64_t num) {
  string full = to_string(num);
  for (size_t i = 1; i <= full.size() / 2; i++) {
    string seq = full.substr(0, i);  // the sequence that must repeat
    bool invalid = true;
    for (size_t j = 0; j < full.size(); j += i) {
      if (full.substr(j, i) != seq) {
        invalid = false;
        break;
      }
    }
    if (invalid) return num;
  }
  return 0;
}
...
int64_t scan_repeats(int64_t start, int64_t stop) {
  ...
  for (int64_t i = start; i <= stop; i++) {
    // for sequence lengths from 1 (max reps) to size / 2 (2 repeats)
    sum += scan_sequence(i);
  }
  return sum;
}
